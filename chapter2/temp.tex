...
这保存当前的训练权重到checkpoint文件。如果训练中断，你可以寻找最新的保存的checkpoint文件使用\bashinline{start_checkpoint=/tmp/speech_commands_train/conv.ckpt-100}命令行参数作为训练开始点。
\subsection{困惑矩阵}
在400步后，信息采集如下:
\begin{pythoncode}
...
\section{验证}
在困惑巨震后，你应该看到这样一行:
\bashinline{I0730 16:57:38.073777 55030 train.py:245] Step 400: Validation accuracy = 26.3% (N=3093)}

一个好的实践是分开你的数据集为三类。对打的(在这个例子中大约80\%的数据)用于训练，小的数据集(10\%用于验证)用于评估训练中的精度，另一个集合(最后的10\%)用于评估训练完成后的精度。

分开的源影视网络将在训练中开始记忆他们的输入。通过设置分开验证，你将确保模型在从没见过的数据上工作。测试集保证你没有训练和验证集扭曲，不是一个输入的边界。

训练脚本自动分开数据集为三类，上面的采集行在验证集上显示模型的精度。最好是这应该接近训练精度。如果训练精度增加但是验证集么有，这是过拟合的信号，你的模型仅仅学习训练的片段不有生成广泛的泛化结果。
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{speech_commands_tensorflow.png}
\caption{训练过程}
\end{figure}
\subsection{训练完成}
在集合晓得训练后(依赖于你的机器的速度)，脚本应该完成所有的18000步。它将打印出一个最后的困惑矩阵，沿着精度的得分，所有的测试集上的运行。结合默认设置，你应该看到精度在85\%到90\%之间。
...
\end{bashcode}
当冻结的模型被创建后，你将使用label\_wav.pu脚本测试，像这样:
\begin{pythoncode}
...
我们通过定义时间窗解决这个问题我们相信我们的说出的关键字将拟合进入，覆盖音频信号到图像中。这通过组合音频采样问题为短的片段，仅仅一些毫秒，计算带宽的频率强度/每个片段的频率强度被当做一个向量，这些向量被以时间排序形成二维数组。数组的值可以被当做单通的图像，被当做一个\href{https://en.wikipedia.org/wiki/Spectrogram}{spectrogram}。如果你想查看图像类别和音频采样输出，你可以运行`wav\_to\_spectrogram`工具。
\begin{bashcode}
...
\bashinline{bash}{bazel run tensorflow/examples/speech_commands:generate_streaming_test_wav}\newline
...
low\_lantency\_svdf基于在\href{https://static.googleusercontent.com/media/research.google.com/en//pubs/archive/43813.pdf}{Compressing Deep Neural Networks using a Rank-Constrained Topology paper},精度将比`conv`小但是仅仅用7500000参数，更搞笑，这允许在测试的时候优化执行(例如当你实际上在你的应用中使用它)，结果有750000FLOPs。
...
\end{bashcode}
注意尽管比之前的量程拓扑要求更多的步骤，减少计算意味着训练应该同时考虑，精度达到85\%。你可以进一步调整拖布计算和在SVDF层改变这参数:
\begin{itemize}
	\item rank 近似排名（越高越好，但是结果是更多的计算。）
	\item num\_units 类似其它层类型，在层中指定阶段属(越多节点，效果越好计算量越大)
\end{itemize}
忽略巡视，因此层允许通过缓存内部神经网络激活优化，你需要确保一个一直的stride(例如`clip\_stride\_ms`标记)然后冻结图，然后在流模型中执行模型（例如，test\_streaming\_accuracy.cc）。

其他参数的自定义，如果你想体验自定义模型，一个好的开始时tweak频谱图创造参数。这影响修改输入图像的大小，创建代码在\href{https://github.com/tensorflow/tensorflow/tree/master/tensorflow/examples/speech_commands/models.py}{models.py}中将调整计算书亮和权重自动拟合不同的维度。如果你使得输入更小，模型将需要更小的计算处理它，因此他可以成为一个折中精度提高时延的方法。--window\_stride\_ms控制之前分析样本的频谱有多远。如果你增加这个值，给定时间内样本越少时间轴输入将缩小，--dct\_coefficient\_count标记控制多少bucket用于瓶铝技术，因此减少这将其他维度缩减输入，--window\_size\_ms参数不影响大小，但是控制每个样本用于计算频谱的区域。减小输入训练样本的维度，你讲需要确保所有的训练数据包含正确的初始片段部分。

如果在你的问题中你有一个完整的不同的模型，你也许可以plug它进入\href{https://github.com/tensorflow/tensorflow/tree/master/tensorflow/examples/speech_commands/models.py}{models.py}有余下的脚本处理预处理和训练机制。你讲添加新的庆幸到create\_model,寻找你的架构的名字，然后调用模型创建函数。这函数给定频谱输入的大小，沿着其他模型信息。希望创建TensorFlow操作读和处理输出预测向量，一个Placeholder控制droupout。余下的脚本处理整个模型为一个大图输入计算使用softmax和损失函数训练它。

一个常见的问题是当你调整模型训练超函数不是一个可以creep的值，可能是数值精度问题。通常你可以通过减少像学习率和权重初始化函数，但是如果他们持续存在你可以使用--check\_nans标记追踪错误元。浙江插入检测操作在TensorFlow操作中，当他们发生后训练将终止。
