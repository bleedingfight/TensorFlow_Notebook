...
data_path = '/home/liushuai/Picture/UCI HAR Dataset'
...
with graph.as_default():
    ...
    drop = td.contrib.rnn.MultiRnnCell([Drop]*lstm_layers)
    initial_state = cell.zeros_state(batch_size,tf.float32)
with graph.as_default():
    ...
    corret_pred = tf.equal(tf.argmax(logits,1),tf.argmax(labels_,1))
    accuracy = tf.reduce_mean(tf.cast(correct_pred,tf.float32),name = 'accuracy')
if (os.path.exists('checkpoints') == False):
    os.mkdir('checkpointers')
validation_acc = []
validation_loss = []
train_acc = []
train_loss = []
...
with tf.Session(graph=graph) as sess:
    ...
    for e in range(epochs):
        ...
        for x,y in get_batch(X_tr,y_tr,batch_size):
